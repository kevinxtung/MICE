...
void Main_Window::keyboardSetup(std::string type) {
    ...
    if (type == "TEXT") {
        ...
        b_space->signal_clicked().connect(sigc::mem_fun(*this, &Main_Window::space));
...
void Main_Window::space() {input += " "; entry->set_text(input);}
...
void Main_Window::dash() {input += "-"; entry->set_text(input);}
